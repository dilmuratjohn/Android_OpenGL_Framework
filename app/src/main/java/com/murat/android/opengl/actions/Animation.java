...
public class Animation {
    ...
    public Animation(Context context, int resource) {
    ...
    private void initTranslateAnimation() {
        ...
        if (null != positionList && positionList.size() > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                mListMove.add(new Actions.Translate(startTime, endTime, startX, startY, (endX - startX), (endY - startY)));
            }
        }
    }
    ...
    private void initRotateAnimation() {
        ...
        if (null != rotationList && rotationList.size() > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                mListAngle.add(new Actions.Angle(startTime, endTime, angle1, (angle2 - angle1)));
            }
        }
    }
    ...
    private void initScaleAnimation() {
        ...
        if (null != scaleList && scaleList.size() > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                mListScale.add(new Actions.Scale(startTime, endTime, scaleX1, scaleY1, (scaleX2 - scaleX1), (scaleY2 - scaleY1)));
            }
        }
    }
    ...
    private void initColorAnimation() {
        ...
        if (null != colorList && colorList.size() > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                mListTint.add(new Actions.Tint(startTime, endTime, r1, g1, b1, (r2 - r1), (g2 - g1), (b2 - b1)));
                mListFade.add(new Actions.Fade(startTime, endTime, a1, (a2 - a1)));
            }
        }
    }
    ...
    private ArrayList<Actions.Translate> mListMove = new ArrayList<>();
    private ArrayList<Actions.Scale> mListScale = new ArrayList<>();
    private ArrayList<Actions.Angle> mListAngle = new ArrayList<>();
    private ArrayList<Actions.Fade> mListFade = new ArrayList<>();
    private ArrayList<Actions.Tint> mListTint = new ArrayList<>();

    public float[] getCurrentTranslate3f() {
        float time = (System.currentTimeMillis() -  mStratTime) / 1000f;
        if (mListMove.size() != 0) {
            Actions.Translate translate = mListMove.get(moveIndex);
    ...
    public float[] getCurrentRotation4f() {
        ...
        if (mListAngle.size() != 0) {
            Actions.Angle angle = mListAngle.get(rotateIndex);
    ...
    public float[] getCurrentScale3f() {
        ...
        if (mListScale.size() != 0) {
            Actions.Scale scale = mListScale.get(scaleIndex);
    ...
    public float[] getCurrentTint3f() {
        float time = (System.currentTimeMillis() -  mStratTime) / 1000f;
        if (mListTint.size() != 0) {
            Actions.Tint tint = mListTint.get(tintIndex);
    ...
    public float[] getCurrentFade1f() {
        float time = (System.currentTimeMillis() -  mStratTime) / 1000f;
        if (mListFade.size() != 0) {
            Actions.Fade fade = mListFade.get(fadeIndex);
            if (time < colorStartTime || time > colorEndTime) {
                return null;
            }
